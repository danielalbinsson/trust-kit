...
function formatIndex(): string {
  ...
  return [
    ...
    "",
    `For MCP resources and tools: ${site.mcpServerUrl}`,
    "",
    "## Promise",
    "",
    "- Inspect: verified self-portrait + honesty contract (Aletheia)",
    "- Gate: authority diff in CI / on deploy (`@aletheia/cli`)",
    ...
    `- [CLI (@aletheia/cli)](${site.cliNpm}): \`npx @aletheia/cli diff --baseline git:main\``,
